...
Generate one interactive HTML page containing every changed file in this Java
project as a side-by-side before/after diff. The page folds long unchanged
runs, highlights changed words within modified lines, lets readers filter
files, and includes collapsed panels for unchanged files.
...
4. Run the bundled generator from the repository root:
   ...
   On systems where the executable is named `python3`, use `python3` instead.
...
5. Confirm the command succeeded and report the absolute path to the generated page. Do not open a browser unless the user asks. Generating the page must not modify the Java source files or Git history.
